...
static int	ft_putnbr_ct(unsigned long n, char format, char *base)
{
	int	base_len;
	int	count;
	int	ret;
	...
	{
		ret = ft_putnbr_ct(n / base_len, format, base);
		if (ret < 0)
			return (-1);
		count += ret;
	}
...
static int	ft_putptr_ct(const void *p)
{
	...
	if (write(1, "0x", 2) < 0)
		return (-1);
	count = ft_putnbr_ct((unsigned long)p, 'p', "0123456789abcdef");
	if (count < 0)
		return (-1);
	return (count + 2);
}
...
static int	ft_format(const char *format, va_list args)
{
	if (*format == 'c')
		...
		return (ft_putnbr_ct(va_arg(args, int), 'i', "0123456789"));
	else if (*format == 'u')
		return (ft_putnbr_ct(va_arg(args, unsigned), 'u', "0123456789"));
	else if (*format == 'x')
		return (ft_putnbr_ct(
				va_arg(args, unsigned), *format, "0123456789abcdef"));
	else if (*format == 'X')
		return (ft_putnbr_ct(
				va_arg(args, unsigned), *format, "0123456789ABCDEF"));
	else if (*format == '%')
