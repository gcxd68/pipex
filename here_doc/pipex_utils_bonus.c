...
void ft_handle_heredoc(t_pipex *data, char *argv[])
{
    char    *line;
    
	ft_memset(data->here_pipe, -1, sizeof(data->here_pipe));
	data->io_fd[1] = open(argv[data->cmd_ct + 3], O_WRONLY | O_CREAT | O_APPEND, 0644);
    if (pipe(data->here_pipe) == -1)
        ft_cleanup(data, NULL, "here_doc pipe failed", 1);
    while (1)
    {
        write(1, "heredoc> ", 9);
